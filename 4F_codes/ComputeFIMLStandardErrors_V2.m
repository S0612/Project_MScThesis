...
models = {
    'GATSM_4F_MacroFinance_Results_KO_proper_V6.mat', 'Model B V6 (KO, 1Q-ahead)', 'Tables/FIML_SE_Model_1Q';
};
...
for m = 1:size(models,1)
    ...
    fprintf('  Parameter vector: %d free parameters\n', nTheta);

    % Verify likelihood at theta0
    logL0 = kf_loglik(theta0, data, xMacro, matSelect, idxExact, idxError, ...
                      nxM, nxL, ann, T);
    fprintf('  logL/T at MCSE estimate: %.6f\n', logL0/T);
    if ~isfinite(logL0)
        warning('logL is not finite at MCSE estimate — check parameter mapping.');
    end
