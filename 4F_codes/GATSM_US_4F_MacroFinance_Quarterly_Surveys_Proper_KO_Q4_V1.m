% This script estimates the four-factor Macro-Finance GATSM with survey augmentation
% using 4-QUARTER-AHEAD SPF forecasts (dpgdp6, UNEMP6).
%
% This is a direct analogue of the 1Q version (KO proper V5):
%   - xMacro = actual macro realisations (unchanged)
%   - xSurvey = dpgdp6 (inflation 4Q-ahead) and UNEMP6 (unemp 4Q-ahead)
%   - Block m OLS augmented by stacking survey rows with weight w_svy
%   - All other steps identical to the 1Q version
...
if w_svy > 0
    ...
    surv_infl_raw  = double(T_infl_raw{:,  'dpgdp6'});   % T x 1  (% ann., 4Q-ahead)
    % Handle 1 NaN in dpgdp6 at 1974Q3, substitute dpgdp5 (3Q-ahead)
    nan_inf = isnan(surv_infl_raw);
    if any(nan_inf)
        surv_infl_3Q   = double(T_infl_raw{:, 'dpgdp5'});
        surv_infl_raw(nan_inf) = surv_infl_3Q(nan_inf);
        fprintf('  Replaced %d NaN(s) in dpgdp6 with dpgdp5.\n', sum(nan_inf));
    end
    surv_unemp_raw = double(T_unemp_raw{:, 'UNEMP6'});   % T x 1  (%, 4Q-ahead)
    % Handle 1 NaN in UNEMP6, substitute UNEMP5 (3Q-ahead)
    nan_une = isnan(surv_unemp_raw);
    if any(nan_une)
        surv_unemp_3Q = double(T_unemp_raw{:, 'UNEMP5'});
        surv_unemp_raw(nan_une) = surv_unemp_3Q(nan_une);
...
if w_svy > 0 && ~isempty(xSurvey)
    % Survey timing for h=4 (dpgdp6/UNEMP6):
    % dpgdp6_t forecasts x_m(t+4). To predict x_m(t+1) we need
    % dpgdp6 made at time t-3, i.e. xSurvey(:, t-3).
    % Valid for OLS rows t=4..T1 only (first 3 rows have no survey).
    % This gives T1_svy = T1-3 survey rows, stacked with all T1 actual rows.
    %
    % For OLS rows t=4..T1 (survey side):
    %   survey dep  = xSurvey(:, t-3)  = xSurvey(:, 1:T1-3)  -> ym_svy
    %   regressor   = xMacro(:, t)     = xMacro(:, 4:T1)     -> xm_svy
    T1_svy = T1 - 3;   % 212 usable survey rows (lose first 3 periods)
    ym_svy = xSurvey(:, 1:T1_svy)';                     % T1_svy x nxM
    xm_svy = [ones(T1_svy,1), xMacro(:, 4:T1)'];        % T1_svy x (1+nxM): cols 4..T1 = 212 cols
...
% Block 2: Y2 on contemporaneous macro and Y1 (unchanged)
y2           = Y2(:, 2:end)';                                      % T1 x Ne
...
% occasionally finds regardless of starting point.  The genuine solution
% has beta_l = [0.00062, 0.00226] (V23-informed) or [0.00234, 0.00043]
% (alternative), both well above 0.0001.  The Jordan swap can permute
% the two entries, so both must be floored to be safe.
lb_B = [-5; -5; -5; 0.0001; 0.0001];
ub_B = [ 5;  5;  5; beta_l_scale(1)*5;  beta_l_scale(2)*5];

% THREE STARTS for robustness — keeps the lowest SSR solution.
% Subproblem B inputs (B1B1_OLS, B2B1_OLS, phiP_ll) are identical to V23
...
% (i.e. the (1,2) off-diagonal of phiQ_ll is <= its (2,1) off-diagonal).
%
...
% with beta_l permuted accordingly. We always keep the representative
...
%
% A2_star and A1_star from OLS are in ANNUAL units.
% local_computeA_ann returns A values in ANNUAL units (multiplied by ann).
% So both sides of the residual equation are in the same units.
...
%   H-W delta0 = -0.0082 quarterly (r0 = -3.3% ann.).
%   The unconditional short rate is E[r] = ann*(alpha + beta'*(I-phiP)^{-1}*muP).
%   With E[y_1yr] ≈ 4.86%, alpha should be in roughly [-0.02, +0.025] quarterly.
%   We use [-0.03, +0.03] to give CMA-ES adequate room without allowing
%   the alpha -> -0.05 degenerate solution that occurs with wider bounds.
%
...
%   H-W values are small (|c^Q| < 0.1).  Allowing ±50 lets Step 7 find
%   large-muQ solutions that imply huge muP_l (same V6/V4 failure mode).
%   Bounding at ±3 prevents this while giving 30× the H-W magnitude.
...
%
% Constraint 2 — muQ shrinkage (from Survey fix):
...
%
% Weight calibration (all residuals scaled to O(1) before squaring):
...
exportgraphics(gcf, 'Graphs/YieldFitPlot_Survey_Q4.pdf');
...
sgtitle('US Yield Curve — Pricing Errors (Actual minus Model-Implied)', ...
        'FontSize', 11, 'FontWeight', 'bold');
exportgraphics(gcf, 'Graphs/PricingErrorPlot_Survey_4Q.pdf');
...
exportgraphics(gcf, 'Graphs/FactorsPlot_Survey_4Q.pdf');

%% LaTeX parameter table

texFilename = 'Tables/GATSM_4F_MacroFinance_Survey_4Q_params.tex';
...
%% Save Data
save('GATSM_4F_MacroFinance_Results_Surveys_Q4.mat', ...
...
for i = 1:ny
    ...
    xlim([dateVec(1) dateVec(end)]); grid on; box on; set(gca,'FontSize',8);
end
sgtitle('US Yield Curve — Term Premia','FontSize',11,'FontWeight','bold');
exportgraphics(gcf,'Graphs/TermPremiaPlot_Survey_4Q.pdf');
...
if ~isempty(selIdx)
    ...
    for k = 1:length(selIdx)
        ...
        hold off;
        title(sprintf('%g-year yield',matYearsDecom(i)),'FontSize',9);
    ...
    sgtitle('US Yield Decomposition','FontSize',11,'FontWeight','bold');
    exportgraphics(gcf,'Graphs/YieldDecompositionPlot_Survey_4Q.pdf');
end
...
exportgraphics(gcf,'Graphs/CS_Survey_4Q.pdf');
...
% hybrid constraint which hopefully matches H-W Table 6 structure
%   Latent Q-block: enforce stationarity |eig(phiQ_ll)| < 0.9999.
%   H-W Table 6 shows latent Q-eigenvalues of [0.887, 0.813] -- both < 1.
...
% Constraint 2: muQ shrinkage
val = val + w_muQ^2 * sum(x(2:end).^2);

% onstraint 3: full unconditional 10yr level
