...
% cmaes').
% Adapted to DSGE models by Martin Møller Andreasen, September 2007
%
...
defopts.StopOnWarnings = 'no   % ''no''==''off''==0, ''on''==''yes''==1 ';
...
if ~flgresume
  ...
  % MMA: we turn off the original boundary handling
  bnd.isactive = 0;
  if bnd.isactive
    if any(lbounds>ubounds)
...
while isempty(stopflag)
  ...
  arx = NaN(N,lambda);
  arxvalid = arx;
  sigma_mp = sigma*ones(lambda,1);
  parfor (k=1:lambda,mp.NumWorkers)
    raw(k,1) = NaN; 
    tries = 0;
    % Resample, until fitness is not NaN
    while isnan(raw(k,1))
      arz(:,k) = randn(N,1);
      arx(:,k) = xmean + sigma_mp(k,1) * (BD * arz(:,k));                % Eq. (1)
      ...
      %for i=1:size(arxvalid(:,k),1)
      %   if arxvalid(i,k) < lbounds(i,1)
      %      ['lower', num2str(i)] 
      %   end
      %   if arxvalid(i,k) > ubounds(i,1)
      %      ['upper', num2str(i)] 
      %   end
      %end
      if any(arxvalid(:,k) < lbounds) == 1 || any(arxvalid(:,k) > ubounds) == 1
          raw(k,1) = NaN;
      else
          raw(k,1) = feval(fitfun, arxvalid(:,k), varargin{:});
      end
      tries = tries + 1;
      if tries==500*lambda 
      	warning(['Many NaN objective function values at evaluation ' ...
		num2str(counteval)]);
        sigma_mp(k,1)= 0.9*sigma_mp(k,1);  
        tries = 0;
        disp(['sigma reduced with 10%. Sigma = ', num2str(sigma)]);
      end
    end
    % NaN are not counted
    %counteval = counteval + 1;
  ...
  if verbosemodulo > 0 & isfinite(verbosemodulo)
    if 1 == 0
        % Existing option of displaying results on the run
        if countiter == 1 | mod(countiter, 10*verbosemodulo) < 1
            disp(['Iterat, #Fevals:   Function Value    (median,worst) ' ...
                '|Axis Ratio|' ...
                'idx:Min SD idx:Max SD']);
        end
        if mod(countiter, verbosemodulo) < 1  | (countiter < 3 & verbosemodulo > 0 & isfinite(verbosemodulo))
    ...
    if  mod(countiter, verbosemodulo) < 1  | (countiter < 3 & verbosemodulo > 0 & isfinite(verbosemodulo))
        prtvec(xmean, N, 'THE MEAN ');
        disp(['BEST VALUE THIS ITERATION: ', num2str(fitness.sel(1),8), ...
            '. BEST VALUE OVERALL: ',num2str(bestever.f,8)]);
        prtvec(sigma*sqrt(diag(C)), N, 'THE STD ');
        disp(['VALUE OF SIGMA: ' num2str(sigma,6)]);
        disp(['NUMBER OF FUNCTION EVALUTIONS: ', num2str(counteval)]);
        disp(['NUMBER OF ITERATIONS: ', num2str(countiter)]);
        disp(' ')
    end
  end
  ...
  if countiter < 10 | ~isempty(stopflag) | ...
